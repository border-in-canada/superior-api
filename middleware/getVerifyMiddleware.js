...
function _getVerifyMiddleware () {
  ...
  return function (req, res, next) {
    ...
    const cookieObj = JSON.parse(cookies.supAuth);
    _verifyMiddleWare(pemsDownloadProm, req, res, cookieObj, next)
  }
}
...
function _verifyMiddleWare (pemsDownloadProm, req, res, authToken, next) {
  const header = `Bearer ${authToken}`;
  pemsDownloadProm.then((pems) => {
    return _verifyProm(pems, header)
  })
    .then((decoded) => {
